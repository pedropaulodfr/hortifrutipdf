...
app.post('/frutas', (req, res) =>{
    client.query("SELECT * FROM frutas ORDER BY nome", (err, results, fields) => {
        console.log(results);
        res.render('frutas', {
            dadosFrutas: JSON.stringify(results),
...
app.post('/verduras', (req, res) =>{
    client.query("SELECT * FROM verduras ORDER BY nome", (err, results, fields) => {
        console.log(results);
        res.render('verduras', {
            dadosVerduras: JSON.stringify(results),
            nomeRota: JSON.stringify(req.route.path)
        })
    })
...
app.post('/legumes', (req, res) =>{
    client.query("SELECT * FROM legumes ORDER BY nome", (err, results, fields) => {
        console.log(results);
        res.render('legumes', {
            dadosLegumes: JSON.stringify(results),
            nomeRota: JSON.stringify(req.route.path)
        })
    })
...
app.post('/autenticacao/:usuario/:senha', (req, res) =>{
    ...
    client.query("SELECT * FROM usuarios", (err, results, fields) => {
        let indexUsuario = results.findIndex(element => element.nome_usuario == usuario)
        let indexSenha = results.findIndex(element => element.senha == senha)
        
        try {
            var usuarioBD = results[indexUsuario].nome_usuario
            var senhaBD = results[indexUsuario].senha
            var idBD = results[indexUsuario].id
    ...
    client.query(`SELECT superuser, id, imagem_perfil FROM usuarios WHERE nome_usuario = '${usuario}'`, (err, results, fields) => {
        ...
        if (results[0].superuser == 1) {
            res.render('painel-admin', {
                superUser: results,
                dadosID: results,
                dadosImagemPerfil: JSON.stringify(results[0].imagem_perfil)
            })
        } else {
            res.render('painel-admin', {
                dadosID: results,
...
app.post('/produtos', (req, res) =>{
    client.query("SELECT * FROM frutas ORDER BY nome", (err, resultsFrutas, fields) => {
        client.query("SELECT * FROM verduras ORDER BY nome", (err, resultsVerduras, fields) => {
            client.query("SELECT * FROM legumes ORDER BY nome", (err, resultsLegumes, fields) => {
                res.render('produtos', {
                    dadosConsultaFrutas: resultsFrutas,
                    dadosConsultaVerduras: resultsVerduras,
                    dadosConsultaLegumes: resultsLegumes
                })
            })
        })
    })
...
app.post('/listar-usuarios', (req, res) => {
    client.query("SELECT * FROM usuarios", (err, resultsUsuarios, fields) => {
        res.render('listar-usuarios', {
            dadosConsultaUsuarios: resultsUsuarios
        })
    })
